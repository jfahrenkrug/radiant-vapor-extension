...
class VaporFlow
  ...
  class << self  
    def call(env)
      ...
      if @@use_regexp
        catch_with_regexp(full_url, relative_url, env['QUERY_STRING'])
      else
        catch_without_regexp(full_url, relative_url, env['QUERY_STRING'])
      end
    end
    
    def catch_with_regexp(full_url, relative_url, query_string)
    ...
    def catch_without_regexp(full_url, relative_url, query_string)
